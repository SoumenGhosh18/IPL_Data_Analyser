...
public class IplAnalyser {
    ...
    public String getTopStrikeRateOftheBowler() throws IplAnalyserException {
        if (censusMap == null || censusMap.size() == 0) {
            throw new IplAnalyserException("NO Census Data", IplAnalyserException.ExceptionType.NO_CENSUS_DATA);
        }
        Comparator<IplDao> censusComparator = Comparator.comparing(census -> census.sr);
    ...
    public String getTopEconomyRateOftheBowler() throws IplAnalyserException {
        if (censusMap == null || censusMap.size() == 0) {
            throw new IplAnalyserException("NO Census Data", IplAnalyserException.ExceptionType.NO_CENSUS_DATA);
        }
        Comparator<IplDao> censusComparator = Comparator.comparing(census -> census.Econ);
        List sortedResult = this.sort(censusComparator.reversed());
        return new Gson().toJson(sortedResult);
    }
    ...
    public String getTopWkctWithBestAvgOftheBowler() throws IplAnalyserException {
        if (censusMap == null || censusMap.size() == 0) {
            throw new IplAnalyserException("NO Census Data", IplAnalyserException.ExceptionType.NO_CENSUS_DATA);
        }
        Comparator<IplDao> censusComparator = Comparator.comparing(census -> census.Wkts + census.Avg);
        List sortedResult = this.sort(censusComparator.reversed());
        return new Gson().toJson(sortedResult);
    }
